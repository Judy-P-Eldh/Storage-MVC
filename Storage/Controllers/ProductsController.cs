...
namespace Storage.Controllers
{
    public class ProductsController : Controller
    {
        ...
        {
            var viewModel = _context.Product.Select(p => new ProductViewModel
            {
                Name = p.Name,
                Price = p.Price,
                Count = p.Count,
        ...
        public async Task<IActionResult>Search(string searchWord, string category)
        {

            var query = string.IsNullOrWhiteSpace(searchWord) ?
                            _context.Product :
                            _context.Product.Where(p => p.Name.StartsWith(searchWord));

            query = string.IsNullOrWhiteSpace(category) ?
                             query :
                             _context.Product.Where(p => p.Category.StartsWith(category));

            var viewModel = query.Select(p => new ProductViewModel
                {
                    Name = p.Name,
                    Price = p.Price,
                    Count = p.Count,
